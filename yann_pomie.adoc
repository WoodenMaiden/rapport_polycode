...
La solution retenue est donc d'utiliser des websockets pour communiquer avec ces services, en effet les websockets permettent des communications asynchrones entre nos services et peuvent passer à travers un proxy. Le seul désavantage de cette solution est qu'elle nécessite une implémentation permettant de relancer les connexions en cas de crash du serveur, chose que le protocole ne fait pas automatiquement.
